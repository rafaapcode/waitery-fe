...
function NotFoundOrgPage() {
  const {
    ...
    fetchingOrgs,
    orgs,
  } = useNotFoundOrgController();

  return (
    <div className="w-1/2 h-full mx-auto flex flex-col justify-center items-center">
      <CreateOrgModal onClose={toggleHandleCreateNew} open={createNewOrg} />
      ...
      <div className="p-6 space-y-4 w-1/2">
        <Activity mode={orgs && orgs.length > 0 ? "visible" : "hidden"}>
        ...
        <Activity
          mode={
            !selectedOrganization && orgs && orgs.length > 0
              ? "visible"
              : "hidden"
          }
        >
        ...
        <div className="flex gap-4">
          ...
          {selectedOrganization && (
            <Button onClick={continueWithOrg} size="sm" className="flex-1">
              Continuar
            </Button>
          )}
        </div>
